...
namespace PERTTool
{
    ...
    public partial class GraphTool : UserControl
    {
        ...
        private void GraphTool_Paint(object sender, PaintEventArgs e)
        {
            ...
            var VisibleNodes = Nodes.Where(x => x.VisibilityCheck(XScroll, YScroll, Height, Width)).ToDictionary(x => x, x => x.GetEdges());
            ...
            foreach (var x in VisibleNodes)
            {
                var k = x.Key.GetEdges();
                if (k.Count == 0) x.Key.Properties["Late Start"] = 0;
                else
                {
                    k.Sort(tcmp);
                    x.Key.Properties["Late Start"] = k.Last().Item1.Properties["Early Start"] - k.Last().Item3;
                }
        ...
        public int PathTime(string path)
        {
            ...
            return Nodes.Select(x => x.GetEdges()) // todas las aristas
        ...
        {
            if (node is null) node = Nodes.FirstOrDefault();
        ...
        private Node PointSelect(float x, float y)
        {
            ...
            {
                RCheckNodes = Nodes.Where(n => n.RangeCheck(x, y)).ToList();
                Node t = RCheckNodes.FirstOrDefault();
                if (t is null) return null;
                RCheckNodes = RCheckNodes.GetRange(1, RCheckNodes.Count - 1);
                lastX = x;
        ...
        private void GraphTool_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            {
                Nodes.Add(new Node(Activity, xPos, yPos));
            }
        ...
        private void GraphTool_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (e.KeyCode == Keys.F && SelectedNode != null)
            {
                ...
                Nodes.ForEach(x => x.DisconnectFrom(SelectedNode));
                SelectedNode = null;
            }
